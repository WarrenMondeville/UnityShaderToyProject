...
float noise(in vec3 x)
{
	...
	vec2 rg = textureLod(iChannel0, (uv + 0.5) / 512).yx;
...
vec4 map(vec3 p)
{
	...
	p.xz = mul(p.xz, mat2(co, -si, si, co));
...
vec3 raymarch(in vec3 ro, in vec3 rd, in vec2 pixel)
{
	...
	// dithering	
	t += 0.05 * textureLod(iChannel0, pixel.xy / 256).x;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	// shake		
	ro += 0.1 * (-1.0 + 2.0 * textureLod(iChannel0, iTime * vec2(0.010, 0.014)).xyz);
	ta += 0.1 * (-1.0 + 2.0 * textureLod(iChannel0, iTime * vec2(0.013, 0.008)).xyz);
