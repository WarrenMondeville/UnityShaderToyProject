...
// Made this shader having not slept for 2 days (was in a game jam) 
// in like 30 minutes. 
...
vec3 glowFog = vec3(0,0,0);
vec3 glowCol = vec3(0,0,0);
...
vec4 valueNoise(float t) {
    return mix(texture(iChannel0, vec2(t,t) / 256.), texture(iChannel0, vec2(t + 1., t + 1.) / 256.), smoothstep(0., 1., fract(t)));
}

vec2 map(vec3 p) {
    vec2 d = vec2(1000, 1000);

    p.xy = mul(p.xy, rot(p.z * 0.3));
    vec3 z = p;
#define modDist 2.
    ...
    for (int i = 0; i < 3; i++) {
        ...
        q.xz = mul(q.xz,rot(0.25 * pi));
        q.yz = mul(q.yz,rot(0.25 * pi));
    }

    float cube = sdBox(q, vec3(0.5, 0.5, 0.5));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    col = pow(col, 0.4545 + dot(uv, uv) * 0.15);
