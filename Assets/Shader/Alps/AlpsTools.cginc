...
#define sunLight normalize(vec3(-0.2, 0.2, -1.0))
#define sunColour vec3(1.0, .88, .75)
float specular = 0.0;
vec3 cameraPos;
float ambient;
...
#define rotate2D mat2(1.6623, 1.850, -1.7131, 1.4623)
...
vec3 PostEffects(vec3 rgb, vec2 uv)
{
	...
	rgb = mix(rgb, dot(rgb, vec3(0.333, 0.333, 0.333)), -1.);
	rgb = sqrt(rgb);
