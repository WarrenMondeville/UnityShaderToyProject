...
vec3 hsv(float h, float s, float v) {
    return mix(1., clamp((abs(fract(h + vec3(3., 2., 1.) / 3.) * 6. - 3.) - 1.), 0., 1.), s) * v;
}
...
vec3 flame_rocket(vec2 uv, vec2 pos, float r, vec2 dir, vec2 rdir) {
    ...
    v = sat(100. * ((0.5 - abs(uv2.x) + flame) - 0.5 * (uv2.y + 0.5)));
...
vec3 starnest(vec2 uv) {
    ...
    for (int r = 0; r < volsteps; r++) {
        ...
        p = abs(frequencyVariation - mod(p, frequencyVariation * 2.0));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    v += clamp(0.2 * worly_star(0.0, uv, uv, 0.2, time), 0., 1.);
