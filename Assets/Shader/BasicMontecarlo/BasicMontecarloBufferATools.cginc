...
float map(vec3 p)
{
    ...
    float d = sdBox(q, vec3(1.0, 1.0, 1.0));
    float s = 1.0;
    for (int m = 0; m < 6; m++)
    {
        float h = float(m) / 6.0;
...
#define sunDir normalize(vec3(-0.3, 1.3, 0.1))
#define sunCol  6.0 * vec3(1.0, 0.8, 0.6)
#define skyCol 4.0 * vec3(0.2, 0.35, 0.5)
...
vec3 calculateColor(vec3 ro, vec3 rd, float sa)
{
    ...
    for (int bounce = 0; bounce < 3; bounce++) // bounces of GI
    {
        ...
        vec3 surfaceColor = vec3(0.4, 0.4, 0.4) * vec3(1.2, 1.1, 1.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec3  rd = normalize(mul(vec3(p, -1.3), ca));

    vec4 data = texture(iChannel0, fragCoord / iResolution.xy);
    if (iFrame == 0) data = vec4(0.0, 0.0, 0.0, 0.0);
