...
#include"SelfieGirlCommon.cginc"
// Created by inigo quilez - iq/2020
// License Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.


// Renders the background (trees, ground, river and bridge). The
// render uses a super basic implementation of Temporal Antialiasing
// (TAA) without color clipping or anything, but it's enough to
// stabilize aliasing. It also outpus the deph buffer into the alpha
// channel for the "Buffer B" pass to consume and do proper Depth Of
// Field (camea defocus).


// The ground where the trees stand. It's a simple sine field.
...
// The bridge. It's made of five boxes (repeated forever with a
// periodic - see the mod() calls), whcih are distorted with gentle
// sine waves so they don't look like geometrically perfect. 
...
// The trees are ultra basic and look really bad without defocus, but
// all I needed was something like looked like pine trees so the
// viewers would complete the picture in their heads. Only four trees
// are evaluated at any time, and there inifinte many of them. Yet
// these four trees consume most of the rendering budget for the scene
//
...
// The SDF of the landscape is made by combining ground, bridge, river
// and trees. 
...
// The landscape SDF again, but with extra high frequency modeling
...
// Computes the normal of the girl's surface (the gradient of the
// SDF). The implementation is weird because of the technicalities
// of the WebGL API that forces us to do some trick to prevent code
// unrolling. More info here:
//
...
// Compute soft shadows for a given light, with a single ray insead of
// using montecarlo integration or shadowmap blurring. More info here:
//
...
// Computes convexity for our landscape SDF, which can be used to
// approximate ambient occlusion. More info here:
//
...
float calcOcclusion(in vec3 pos, in vec3 nor, in float time, float sca, in vec2 px)
{
    float kk; vec3 kk2;
    float ao = 0.0;
    float off = textureLod(iChannel2, px / 256.0).x;
...
// Computes the intersection point between our landscape SDF and a ray
// (coming form the camera in this case). It's a traditional and
// basic/uncomplicated SDF raymarcher. More info here:
//
// https://iquilezles.org/www/material/nvscene2008/rwwtt.pdf
//
...
// Renders the landscape. It finds the ray-landscape intersection point,
// computes the normal at the intersection point, computes the ambient
// occlusion approximation, does per material setup (color, specularity,
// and paints some fake occlusion), and finally does the lighting
// computations themseleves.
...
vec4 renderBackground(in vec2 p, in vec3 ro, in vec3 rd, in float time, in vec2 px)
{
    ...
    if (tm.y > 0.0)
    {
        ...
        vec3  amb = occ * float(0.55 + 0.45 * nor.y);
...
// The main rendering entry point. Basically it does some setup
// or creating the ray that will explore the 3D scene in search
// of the landscape for each pixel, does the rendering of the
// landscape, and performs the Temporal Antialiasing before
// spiting out the color (in linear space, not gama) and the deph
// of the scene.
//
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec3 rd = mul(ca, normalize(vec3((p - vec2(-0.52, 0.12)) / 1.1, fl)));
    ...
    mat4 oldCam = mat4(textureLod(iChannel1, vec2(0.5, 0.5) / iResolution.xy),
        textureLod(iChannel1, vec2(1.5, 0.5) / iResolution.xy),
        textureLod(iChannel1, vec2(2.5, 0.5) / iResolution.xy),
        0.0, 0.0, 0.0, 1.0);
    bool oldStarted = textureLod(iChannel1, vec2(3.5, 0.5) / iResolution.xy).x > 0.5;

    // world space
    vec4 wpos = vec4(ro + rd * tmp.w, 1.0);
    // camera space
    vec3 cpos = (mul(wpos, oldCam)).xyz; // note inverse multiply
    // ndc space
    ...
    {
        vec3 ocol = textureLod(iChannel1, spos).xyz;
