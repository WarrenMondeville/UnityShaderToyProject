...
vec3 makeBloom(float lod, vec2 offset, vec2 bCoord) {
    ...
    //if (any(greaterThanEqual(abs(coord - 0.5), scale + 0.5)))
        //return 0.0;
    if (abs(coord - 0.5).x > (scale + 0.5).x || abs(coord - 0.5).y > (scale + 0.5).y)
    {
        return 0.0;
    }
    for (int i = -5; i < 5; i++) {
        for (int j = -5; j < 5; j++) {
            ...
            bloom = pow(texture(iChannel0, vec2(i, j) * scale + pixelSize + coord).rgb, 2.2) * wg + bloom;
            totalWeight += wg;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    fragColor = vec4(pow(blur, 1.0 / 2.2), 1.0);
}