...
vec3 hash3(uint n)
{
    ...
    return 0;//vec3(k & 0x7fffffffU) / float(0x7fffffff);
}
...
{
    return textureLod(tex, (x + 0.5) / 32.0).x;
}
...
#define X4 1.1673039782614187
#define H4 vec4(1.0 / X4, 1.0 / (X4 * X4), 1.0 / (X4 * X4 * X4), 1.0 / (X4 * X4 * X4 * X4));
