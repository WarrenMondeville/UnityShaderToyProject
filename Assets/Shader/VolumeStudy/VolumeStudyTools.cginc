...
#define MAX_SDF_SPHERE_STEPS 10
...
CameraDescription Camera = CameraDescription(vec3(0, 100, -265), vec3(0, 5, 0), 2.0, 7.0);

vec3 GetLightColor(int lightIndex)
{
    switch (lightIndex % NUM_LIGHT_COLORS)
    {
    case 0: return vec3(1., 0.0, 0.0);
...
float VolumetricDistanceField(in vec3 pos)
{
    vec3 fbmCoord = (pos + 2.0 * vec3(iTime, 0.0, iTime)) / 1.5f;
    float sdfValue;
    sdfValue = sdSphere(pos, vec3(-8.0, 10.0, -1), 5.6);
    sdfValue = sdSmoothUnion(sdfValue, sdSphere(pos, vec3(8.0, 2.0 + 12.0 * cos(iTime), 3), 5.6), 3.0f);
    sdfValue = sdSmoothUnion(sdfValue, sdSphere(pos, vec3(50.0 * sin(iTime), 3.0, 0), 8.0), 3.0) + 7.0 * fbm_4(fbmCoord / 3.2);

    sdfValue = sdSmoothUnion(sdfValue, sdPlane(pos + vec3(0, 9.4, 0)), 22.0);
    return sdfValue;
}
...
float IntersectVolumetric(in vec3 rayOrigin, in vec3 rayDirection, float maxT)
{
    ...
    {
        float result = VolumetricDistanceField(rayOrigin + rayDirection * t);
...
float GetLightVisiblity(in vec3 rayOrigin, in vec3 rayDirection, in float maxT, in int maxSteps, in float marchSize)
{
    ...
    for (int i = 0; i < maxSteps; i++)
    {
        ...
        signedDistance = VolumetricDistanceField(position);
...
void CalculateLighting(vec3 position, vec3 normal, vec3 reflectionDirection, Material material, inout vec3 color)
{
    for (int lightIndex = 0; lightIndex < NUM_LIGHTS; lightIndex++)
    {
        ...
        vec3 D;
        Diffuse2(normal, lightDirection, material.albedo, D);
        color += lightVisiblity * lightColor * D;
    }
...
vec3 Render(in vec3 rayOrigin, in vec3 rayDirection)
{
    ...
    if (volumeDepth > 0.0)
    {
        const vec3 volumeAlbedo = vec3(1.0);
    ...
    if (materialID != INVALID_MATERIAL_ID && opaqueVisiblity > ABSORPTION_CUTOFF)
    {
        ...
        // opaqueColor = min(material.albedo, vec3(0.0));
