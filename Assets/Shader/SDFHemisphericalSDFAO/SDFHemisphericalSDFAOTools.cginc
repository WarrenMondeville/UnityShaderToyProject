...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	rd = mul(rd, lookat(-ro + vec3(0.0, 5.0, 0.0), vec3(0.0, 1.0, 0.0)));
...
float map(in vec3 p)
{
	float d = -box(p - vec3(0., 10., 0.), vec3(10., 10., 10.));
	d = min(d, box(mul(rotate(vec3(0., 1., 0.), 1.), (p - vec3(4., 5., 6.))), vec3(3., 5., 3.)));
	d = min(d, box(mul(rotate(vec3(0., 1., 0.), -1.), (p - vec3(-4., 2., 0.))), vec3(2., 2., 2.)));
	d = max(d, -p.z - 9.);
