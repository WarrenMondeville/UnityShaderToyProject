...
float pn(in vec3 p)
{
    ...
    uv = texture(iChannel0, (uv + 0.5) / 256.0).yx;
...
float Star(vec3 position, vec3 rotdir, vec3 rposition, float radius)
{
    ...
    float noise = Turbulence(vec3(mul(Spin(t * 0.25 + rotdir.y * .81) , rotdir.zx) * 50., rotdir.y * 50. + t), 0.1, 1.5, 0.03) * NoiseAmplitude;
...
float Vortex(vec3 q) // the isosurface shape function, the surface is at o(q)=0
{
    ...
    vec3 spin_pos = vec3(mul(Spin(t - sqrt(q.y)) , q.xz), q.y - t * 5.0);
...
float map(vec3 p) {
    ...
    float d3 = Disk(p, vec3(1.0, 0.98, 0.322)) + fpn(vec3(mul(Spin(t * 0.125 + p.z * .80), p.xy) * 80., p.z * 40. - t) * 6.0) * 0.545;
...
vec3 firePalette(float i) {
    ...
    L = pow(L, vec3(5.0,5.0,5.0)) * (exp(1.43876719683e5 / (T * L)) - 1.0);
...
// rm loop
...
#ifdef DITHERING 
        d = abs(d) * (.8 + 0.28 * rand(seed * vec2(i,i)));
#endif 
    ...
    // stars background
    if (ld <= 0.001) {
        vec3 stars = pn(rd * 300.0) * 0.5 + 0.5;
        vec3 col = vec3(0.0,0.0,0.0);
        col = mix(col, vec3(0.8, 0.9, 1.0), smoothstep(0.95, 1.0, stars) * clamp(dot(0.0, rd) + 0.75, 0.0, 1.0));
