...
#define frameToRenderHQ 50.0 // Time in seconds of frame to render
#define antialiasingSamples 16.0 // 16x antialiasing - too much might make the shader compiler angry.
...
vec3 GetEnvMap(vec3 rayDir, vec3 sunDir)
{
    ...
    finalColor = mix(finalColor, (vec3(1.0, 1.0, 1.0) + sunCol * 10.0) * 0.75 * saturate((rayDir.y + 0.2) * 5.0), saturate(n * 0.125));
...
vec3 RayTrace(in vec2 fragCoord)
{
    ...
    if ((t <= maxDepth) || (t == alpha))
    {
        ...
        if ((normal.y < 0.1) && (distAndMat.y == 0.0))
        {
            ...
            vec3 colTotal = vec3(0.0, 0.0, 0.0);
            vec3 colTemp = texColor;
            vec3 nTemp = vec3(0.0, 0.0, 0.0);
        ...
        else
        {
            ...
            texColor = mix(texColor, vec3(0.5, 0.5, 0.5), whiteLine);
            ...
            crossWalk *= n * n;
            texColor = mix(texColor, vec3(0.25, 0.25, 0.25), crossWalk);
...

            {
                // sidewalk cracks
                float sidewalk = 1.0;
                vec2 blockSize = vec2(100.0, 100.0);
        ...
        if (distAndMat.y == 3.0)
        {
            texColor = vec3(0.05, 0.05, 0.05);
        }

        // apply noise
        texColor *= vec3(1.0, 1.0, 1.0) * n * 0.05;
        texColor *= 0.7;
        ...
        if (distAndMat.y >= 100.0)
        {
            ...
            texColor = pow(abs(texColor), vec3(0.2, 0.2, 0.2));  // bias toward white
            texColor = max(vec3(0.25, 0.25, 0.25), texColor);  // not too saturated color.
        ...
        vec3 lightColor = vec3(100.0, 100.0, 100.0) * sunCol * saturate(dot(sunDir, normal)) * sunShadow;
        ...
        fogColor = min(vec3(9.0, 9.0, 9.0), fogColor);
    ...
    // vignette?
    finalColor *= vec3(1.0, 1.0, 1.0) * saturate(1.0 - length(uv / 2.5));
