...
#define c_grassDistance 0.25 // how far apart on the x axis
#define c_grassMaxDepth 1.0  // how far below ground level it can go

#define c_treeDistance 3.0 // how far apart on the x axis
#define c_treeMaxDepth 0.5 // how far below ground level it can go

#define c_cloudDistance 2.0 // how far apart on the x axis
#define c_cloudMaxDepth 2.0 // vertical offset. +/- this amount max
...
vec2 AsyncPointPos(in vec4 point2, in float frameFraction)
{
    return mix(point2.zw, point2.xy, frameFraction);
}
...
float EstimatedDistanceFromPointToGround(in vec2 point2, float scale)
{
    float v = GroundFunction(point2, scale);
    vec2  g = GroundFunctionGradiant(point2, scale);
    return v / length(g);
}

float EstimatedDistanceFromPointToGround(in vec2 point2, float scale, float frequencyScale, out vec2 gradient)
{
    float v = GroundFunction(point2, scale);
    gradient = GroundFunctionGradiant(point2, scale);
...
vec4 loadValue(in vec2 re)
{
    ...
    return texture(iChannel0, (0.5 + re) / _ScreenParams.xy);
}
...
{
    vec3 wheelColor = vec3(0.0,0.0,0.0);
    ...
    zoomCircleDist = 1.0 - smoothstep(0.0, AA_AMOUNT, zoomCircleDist);
    pixelColor = mix(pixelColor, vec3(0.75, 0.75, 0.75), zoomCircleDist);
}
...
void DrawGround(in vec2 uv, in vec2 cameraOffset, inout vec3 pixelColor, vec4 backWheel, vec4 frontWheel, vec4 state, vec4 state2, vec4 state3)
{
    ...
    DrawHills(uv + vec2(1000.0, -0.3) + cameraOffset * -0.9, pixelColor, vec3(0.25, 0.25, 0.25), 0.7);
    DrawClouds(uv + vec2(1000.0, -0.3) + iTime * vec2(0.05, 0.0) + cameraOffset * -0.85, pixelColor, vec3(0.3, 0.3, 0.3), 0.75, 0.75);
    DrawHills(uv + vec2(300.0, -0.1) + cameraOffset * -0.8, pixelColor, vec3(0.5, 0.5, 0.5), 0.8);
    DrawClouds(uv + vec2(300.0, -0.1) + iTime * vec2(0.15, 0.0) + cameraOffset * -0.7, pixelColor, vec3(0.6, 0.6, 0.6), 0.75, 0.75);
    ...
    DrawHills(uv, pixelColor, vec3(1.0, 1.0, 1.0), 1.0);
    ...
    DrawClouds(uv + vec2(700.0, -1.25) + iTime * vec2(0.25, 0.0) + cameraOffset * 0.5, pixelColor, vec3(1.0, 1.0, 1.0), 1.0, 0.5);
}
...
void DrawSpeedometer(vec2 fragCoord, float speedPercent, inout vec3 pixelColor)
{
    ...
    zoomCircleDist = 1.0 - smoothstep(0.0, AA_AMOUNT, zoomCircleDist);
    pixelColor = mix(pixelColor, vec3(0.1, 0.1, 0.1), zoomCircleDist);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (0 && VAR_GAMEOVER == 1.0)
    {
        vec3 greyPixel = dot(pixelColor, vec3(0.3, 0.59, 0.11));
        pixelColor = mix(vec3(1.0, 0.0, 0.0), greyPixel, min(1, VAR_SIMSLOWDOWN * 0.75 + 0.25));
    }
