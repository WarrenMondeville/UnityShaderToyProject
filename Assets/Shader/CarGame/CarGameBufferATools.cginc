...
#define c_tickRate 16.0

// what factor to slow the game down by when it's game over
#define c_tickRateGameOver 0.0

// how long the game over state slows down the simulation for
#define c_gameOverSlowdownDuration 3.0

// how many simulation steps to do per tick.  More = more costly, but better simulations.
#define c_numSimulationSteps 1

// simulation constants
#define c_gravityAcceleration vec2(0.0, -5.0)  // -9.8 is real life values, but 1 unit != 1 meter in our sim.
#define c_rotationMultiplier 0.1 // how much rotation done
#define c_throttleAcceleration 4.0
#define c_fuelBurnRate 10.0
#define c_checkPointDistance 5.0 // in world units, distance between checkpoints.

// derived values
#define c_tickDeltaTime 0.0625 //1.0 / c_tickRate
#define c_tickDeltaTimeSq 0.00390625 //c_tickDeltaTime * c_tickDeltaTime

// keys
#define KEY_SPACE 32.5 / 256.0
...
#define PIOVERTWO 1.570796326795 //(PI * 2.0)
#define TWOPI 6.28318530718 //(PI * 2.0)

#define c_wheelRadius 0.04
#define c_wheelDistance 0.125

#define c_fuelCanDistance 20.0
#define c_fuelCanRadius 0.075

// variables
#define txState vec2(0.0, 0.0)
...
#define txFrontWheelCP1 vec2(3.0, 0.0)
#define txFrontWheelCP2 vec2(4.0, 0.0)
#define txBackWheelCP1 vec2(5.0, 0.0)
#define txBackWheelCP2 vec2(6.0, 0.0)
#define txState4 vec2(7.0, 0.0)
// x = fuel at CP1
// y = fuel at CP2
// z = last CP hit
// w = unused
...
#define txBackWheel vec2(8.0, 0.0)
#define txFrontWheel vec2(9.0, 0.0)

#define txVariableArea vec2(10.0, 1.0)
...
vec2 AsyncPointPos(in vec4 point1, in float frameFraction)
{
    return mix(point1.zw, point1.xy, frameFraction);
}
...
float EstimatedDistanceFromPointToGround(in vec2 point1, float scale)
{
    float v = GroundFunction(point1, scale);
    vec2  g = GroundFunctionGradiant(point1, scale);
    return v / length(g);
}

float EstimatedDistanceFromPointToGround(in vec2 point1, float scale, float frequencyScale, out vec2 gradient)
{
    float v = GroundFunction(point1, scale);
    gradient = GroundFunctionGradiant(point1, scale);
...
vec4 loadValue(in vec2 re)
{
    ...
    return texture(iChannel0, (0.5 + re) / iResolution.xy);
}
...
//============================================================
vec2 RotatePoint(vec2 point1, float theta)
{
    return vec2(point1.x * cos(theta) - point1.y * sin(theta), point1.y * cos(theta) + point1.x * sin(theta));
}

//============================================================
vec2 RotatePointAroundPoint(vec2 point1, vec2 origin, float theta)
{
    return RotatePoint(point1 - origin, theta) + origin;
}

//============================================================
void VerletIntegrate(inout vec4 point1, in vec2 acceleration)
{
    vec2 currentPos = point1.xy;
    vec2 lastPos = point1.zw;

    vec2 newPos = currentPos + currentPos - lastPos + acceleration * c_tickDeltaTimeSq;

    point1.xy = newPos;
    point1.zw = currentPos;
}

//============================================================
void ResolveGroundCollision(inout vec2 point1, inout bool pointTouchingGround)
{
    vec2 gradient;
    float dist = EstimatedDistanceFromPointToGround(point1, 1.0, 1.0, gradient) * -1.0;
    if (dist < c_wheelRadius)
    {
        float distanceAdjust = c_wheelRadius - dist;
        point1 -= normalize(gradient) * distanceAdjust;
