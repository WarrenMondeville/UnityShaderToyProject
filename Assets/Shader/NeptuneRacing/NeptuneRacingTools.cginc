...
float Noise(in vec3 x)
{
    ...
    vec2 rg = textureLod(iChannel2, (uv + 0.5) / 256.0).yx;
...
{
    float tx1 = smoothstep(0., .4, textureLod(iChannel0, 0.000015 * q).y);
    tx1 = mix(tx1, textureLod(iChannel1, 0.00003 * q).x, tx1);
    return tx1 * 355.0;
}
...
vec3 GetClouds(vec3 p, vec3 dir)
{
    ...
    vec3 clo = textureLod(iChannel3, p2 * .00001 + .2).zyz * .04;
    n = (1000.0 - p.y) / dir.y;
    p2 = p.xz + dir.xz * n;
    clo += textureLod(iChannel0, p2 * .00001 - .4).zyz * .04;
...
vec3 PostEffects(vec3 rgb, vec2 xy)
{
    ...
    rgb = mix(.5, mix(dot(vec3(.2125, .7154, .0721), rgb * BRIGHTNESS), rgb * BRIGHTNESS, SATURATION), CONTRAST);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (alpha < .8)
    {
        ...
        {
            float stars = pow(texture(iChannel2, vec2(atan(dir.x, dir.z), dir.y * 2.0)).x, 48.0) * .35;
