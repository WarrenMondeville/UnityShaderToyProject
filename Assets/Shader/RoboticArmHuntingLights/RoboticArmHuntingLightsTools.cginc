...
vec3 rotateY(const in vec3 p, const in float t) {
    float co = cos(t);
    float si = sin(t);
    vec2 xz = mul(mat2(co, si, -si, co), p.xz);
...
vec3 getEmissive(in vec2 pos, in float mat) {
    if (mat > 2.5) {
        ...
        if (li0 > 0. && pos.x == activeSpheres[0].x && pos.y == activeSpheres[0].y) {
            return sphereCol(time) * li0 * 1.25;
        }
        else if (li1 > 0. && pos.x == activeSpheres[1].x && pos.y == activeSpheres[1].y) {
...
vec3 render(in vec3 ro, in vec3 rd) {
    ...
    for (int i = ZERO; i < PATH_LENGTH; ++i) {
        ...
        if (res.z > 0.) {
            ...
            if (res.z < 3.5) {
                vec3 F = FresnelSchlick(0.4, normal, -rd);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 mo = iMouse.xy == 0 ? vec2(.4, -.1) : abs(iMouse.xy) / iResolution.xy - .5;
...
#endif
    vec3 rd = mul(ca ,normalize(vec3(p.xy, 1.6)));
    col += pow(8. * render(ro, rd), 1. / 2.2);
#if AA>1
        }
col /= float(AA * AA);
#endif

col = clamp(col + ((hash22(fragCoord).x - .5) / 64.), 0, 1);
