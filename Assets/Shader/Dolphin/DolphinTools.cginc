...
#define m2 mat2(0.80, -0.60, 0.60, 0.80)
...
vec2 sdWaterCheap(vec3 p)
{
	vec2 q = 0.05 * p.xz;
	float f = 0.0;
	f += 0.50000 * almostAbs(noise(q)); q = mul(q, m2) * 2.02; q -= 0.1 * iTime;
	f += 0.25000 * almostAbs(noise(q)); q = mul(q, m2) * 2.03; q += 0.2 * iTime;
	f += 0.12500 * almostAbs(noise(q)); q = mul(q, m2) * 2.01; q -= 0.4 * iTime;
	f += 0.06250 * almostAbs(noise(q)); q = mul(q, m2) * 2.02; q += 1.0 * iTime;
...
#define lig vec3(0.86, 0.15, 0.48)
