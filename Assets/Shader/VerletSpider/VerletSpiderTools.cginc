...
float mapDE(vec3 p)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        d = min(d, lowerLeg(p, spiderLegs[i].ankle, spiderLegs[i].point1, .09, .05));
    }
...
vec3 mapCE(vec3 p)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        h = max(lowerLegCE(p, spiderLegs[i].ankle, spiderLegs[i].point1, .09, .05), h);
        if (h > .0)
...
void moveSpider()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        spiderLegs[i].point1 = spiderLegs[i].fix;
        spiderLegs[i].point1.y += sin(az * 3.141); // az*(1.0-az)*4.0;//
        spiderLegs[i].point1.z += (iz * 3.0 + az * 3.0 - h * 4.) + (s < .0 ? 1.5 : 0.);
        spiderLegs[i].fix = spiderLegs[i].fix * vec3(.12, .4, .1) + body - vec3(.0, .34, 0.);
        spiderLegs[i].knee = (spiderLegs[i].point1 + spiderLegs[i].fix) * .5;
        ...
        verlet(spiderLegs[i].point1, spiderLegs[i].knee, 2.2);

        spiderLegs[i].ankle = (spiderLegs[i].point1 + spiderLegs[i].knee) * .5;
...
void mainImage(out vec4 outColour, in vec2 coords)
{
    ...
    else
    {
        ...
        if (dir.y < 0.0)
        {
            ...
            mat = mix(mat, vec3(0.0, 0.0, 0.0), smoothstep(0., .025, f) * smoothstep(.05, .025, f) * .75);
        }
        else
        {
