...
float de(in vec3 p, in vec3 dir, in float r, out vec3 color) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        // colors and light
        vec3 albedo = sin(sphere.x * 90.0 + sphere.y * 80.0) * 0.45 + 0.5;
        vec3 colorHere = 0;

        if (dot(toRed, toRed) < 0.001) {
            albedo = mix(albedo, 0.0, fRed);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    dir.yz = mul(rot(-0.85), dir.yz);
    dir.xy = mul(rot(0.2),dir.xy);
    ...
    fragColor.rgb = clamp(acc.rgb, 0, 1);
