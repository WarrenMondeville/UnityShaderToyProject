...
float sceneIntersection(in vec3 ro, in vec3 rd, out vec3 inter, out vec3 normal, out vec3 color, in float dist, out vec3 lightPos)
{
    ...
    // Floor
    {
        ...
        if (t > 0. && t < mint)
        {
            ...
            color = ((tileId.x & 1) ^ (tileId.y & 1)) == 0 ? 0.3 : 0.15;
        }
    }
    return mint;
}
...
vec3 Render(in vec3 ro, in vec3 rd, in float dist, float fudge)
{
    ...
    if (floorIntersect(ro, rd, fogHeigth, t))
    {
        ...
        color = mix(color, lightEnergy, 1. - transmittance);
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec3 rd = mul(normalize(vec3(p, 1.5)), ca);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float3 tFog = texelFetch(iChannel0, ivec2(fragCoord) / float2(_ScreenParams.xy)).z;
    fragColor = vec4(tFog, 1);
}
#endif	// RENDER3D