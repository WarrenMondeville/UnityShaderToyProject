...
float fBox(vec3 p, vec3 b) {
    vec3 d = abs(p) - b;
    return length(max(d, vec3(0, 0, 0))) + vmax(min(d, vec3(0, 0, 0)));
}
...
# endif
    ...
    d *= .6;
    Model tModel = (Model)0;
    tModel.dist = d;
    tModel.colour = colour;
    tModel.id = 1;
    return tModel;
}
...
#define MAX_TRACE_DISTANCE 30.0           // max trace distance
#define INTERSECTION_PRECISION 0.001        // precision of the intersection
...
void doCamera(out vec3 camPos, out vec3 camTar, out float camRoll, in float time, in vec2 mouse) {
    ...
    camPos = mul(camPos, m);
    camPos.y += height;
    camTar = -camPos + vec3(.0001, .0001, .0001);
    camTar.y += height;
    camRoll = 0.;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // create view ray
    vec3 rd = normalize(mul(camMat, vec3(p.xy, 2.0))); // 2.0 is the lens length
