...
vec3 getColor(vec3 p)
{
    ...
    if (length(p) > 0.99)return vec3(0., 0., 0.);
    p = abs(p);
    ...
    out bool si, out float tsi, out vec3 normsi, out float fade, out float fadesi)
{
    ...
    if (abs(p) < 0.000001)
    {
        ...
        vec3 pos = ro + t * rd;
        vec3 grad =
            vec3(2.0, 2.0, 2.0) * pos.xzy * vec3(a, b, c) + pos.zxz * vec3(d, d, e) + pos.yyx * vec3(f, e, f) + vec3(g, h, i);
        norm = -normalize(grad);
        return true;
    }
    else
    {
        ...
        else
        {
            ...
            if ((tt2 > 0.) && ((!ru)) && !(step(sz, length(ro + tt2 * rd)) > 0.5))
            {
                ...
                vec3 tpos = ro + tsi * rd;
                // normal
                vec3 tgrad = vec3(2.0, 2.0, 2.0) * tpos.xzy * vec3(a, b, c) + tpos.zxz * vec3(d, d, e) +
...
{
    return mix(vec3(0.01, 0.01, 0.01), vec3(0.336, 0.458, .668), 1. - pow(abs(rd.z + 0.25), 1.3));
}
...
vec3 background(in vec3 ro, in vec3 rd, vec3 l_dir)
{
    ...
    float shad = boxSoftShadow((ro + t * rd), mul(normalize(l_dir + vec3(0., 0., 1.)), rotz(PI * 0.65)), BOXDIMS, 1.5);
    float aofac = smoothstep(-0.95, .75, length(abs(uv) - min(abs(uv), vec2(0.45, 0.45))));
    aofac = min(aofac, smoothstep(-0.65, 1., shad));
    float lght = max(dot(normalize(ro + t * rd + vec3(0., -0., -5.)), mul(normalize(l_dir - vec3(0., 0., 1.)), rotz(PI * 0.65))), 0.0);
    vec3 col = mix(vec3(0.4, 0.4, 0.4), vec3(.71, .772, 0.895), lght * lght * aofac + 0.05) * aofac;
...
vec4 insides(vec3 ro, vec3 rd, vec3 nor_c, vec3 l_dir, out float tout)
{
    ...
    dx[2] = vec3(-1., -1., -1.);
    vec4 colxsi[3];
    colxsi[0] = vec4(0., 0., 0., 0.);
    colxsi[1] = vec4(0., 0., 0., 0.);
    colxsi[2] = vec4(0., 0., 0., 0.);
    int order[3];
    order[0] = 0;
    order[1] = 1;
    order[2] = 2;
    ...
    // self intersection
    bool rul[3];
    rul[0] = (dx[0].y > 0.5) && (dx[1].x <= 0.);
    rul[1] = (dx[1].y > 0.5) && (dx[0].x > dx[1].z);
    rul[2] = (dx[2].y > 0.5) && (dx[1].x > dx[2].z);

    for (int k = 0; k < 3; k++) {
        if (rul[k]) {
            vec4 tcolxsi = vec4(0., 0., 0., 0.);
            tcolxsi = colxsi[order[k]];
            vec4 tcolx = vec4(0., 0., 0., 0.);
            tcolx = colx[order[k]];

            vec4 tvalx = mix(tcolxsi, tcolx, tcolx.a);
            colx[order[k]] = tvalx;

            vec4 tvalx2 = mix(vec4(0., 0., 0., 0.), tvalx, max(tcolx.a, tcolxsi.a));
            colx[order[k]] = tvalx2;
        }
    }
