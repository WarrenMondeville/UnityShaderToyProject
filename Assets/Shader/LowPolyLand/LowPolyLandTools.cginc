...
#define aaTreshold 4.0 // size of edge for anti-aliasing
...
float polyfbm(vec2 p)
{
    ...
    m1 += polynoise(mul((vec2(0.5, 0.5)) + p, r1));
    m1 += polynoise(mul((vec2(0.35, 0.415)) + p, r3));
    m1 *= 0.33;

    float m2 = polynoise(mul((p * 2.4), r3));
    m1 += m2 * 0.05;
    return m1;
}
...
void trace(in vec3 rp, in vec3 rd, inout vec4 color)
{
    ...
    bool hit = false;
    vec3 ro = rp;
    vec4 bgcolor = texture(iChannel0, mul(rd , roty(iTime * 0.2))) * vec4(0.2, 0.2, _uv.y + 0.4, 0.0);

    float closest = 999.0;
    vec3 closestPoint = 0.0;
    float dist = 0.0;
    for (int i = 0; i < 400; ++i)
    {
