...
void init(vec2 fragCoord) {
    ...
    _stack.line_width = 1.0;
    _stack.blur = vec2(AA, 0.0);
    _stack.source = vec4(0.0, 0.0, 0.0, 1.0);
    _stack.start_pt = vec2(0.0, 0.0);
    _stack.last_pt = vec2(0.0, 0.0);
}
...
void set_matrix(mat3 mtx) {
    ...
    _stack.position = (mul(mtx, vec3(position, 1.0))).xy;
    _stack.scale = length(vec2(mtx[0].x, mtx[1].y));
}

void transform(mat3 mtx) {
    mtx = mat2x3_invert(mtx);
    _stack.position = (mul(mtx , vec3(_stack.position, 1.0))).xy;
