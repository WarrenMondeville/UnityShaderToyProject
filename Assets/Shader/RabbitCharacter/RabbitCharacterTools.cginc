...
float cylinder(vec3 p, vec3 dir, float h, float r)
{
    ...
    return length(max(abs(vec2(d, t)) - vec2(r, h), vec2(0, 0)));
    d = max(d, -t);
    d = max(d, t - h);
    return d;
}
...
float rabdist(vec3 p)
{
    ...
    p.xz = mul(mat2(cos(an), sin(an), sin(an), -cos(an)), p.xz);

    float time2 = time * 2.4;

    p.y += 2.3;
    p.xy = mul(p.xy, rotmat(cos(time2 + 1.) * .04));
    p.y -= 2.3;

    vec3 op = p;

    vec3 p2 = p;
    p2.xy = mul(p2.xy, rotmat(cos(time2) * .1));

    vec3 p3 = p;
    p3.xy = mul(p3.xy, rotmat(cos(time2 - .0 - length(p) / 2.) * .13));
    ...
    float ld = -.75;
    p4.y -= ld;
    p4.yz = mul(p4.yz, rotmat(cos(time * 2. + 3.1415926 / 1. * 0.) * .1));
    p4.y += ld;
    p4.y -= max(0., cos(time * 2. + 3.1415926 / 2.)) * .1;
    d = smin(d, distance(vec3(.3, clamp(p4.y, -2.6, -2.), 0.), p4.xyz) - .3, .1);
    p4 = op;
    p4.y -= ld;
    p4.yz = mul(p4.yz, rotmat(cos(time * 2. + 3.1415926 / 1. * 1.) * .1));
    p4.y += ld;
    ...
    p.y += .2;
    p.xy = mul(p.xy, rotmat(.4 + cos(time2 * 2.) * .02));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (t < 10.)
    {
        ...
        if (floordist(rp) < rabdist(rp))
            col = max(mix(bcol + (.1 - length(vt.xy) / 3.), vec3(1, 1, 1), .1), 0.);
        ...
        col = .84 * max(mix(bcol + (.1 - length(vt.xy) / 3.), vec3(1, 1, 1), .1), 0.);
