...
vec4 map(in vec3 pos, float atime)
{
    ...
    float rc = cos(rot);
    float rs = sin(rot);
    q.xy = mul(mat2x2(rc, rs, -rs, rc) , q.xy);
    vec3 r = q;
    href = q.y;
    ...
    if (res.x - 1.0 < pos.y) // bounding volume
    {
        ...
        h.xz = mul(mat2x2(cos(hr), sin(hr), -sin(hr), cos(hr)) , h.xz);
        ...
        // eye
        {
            ...
            cq.xy = mul(mat2x2(0.8, 0.6, -0.6, 0.8), cq.xy);
...
vec3 render(in vec3 ro, in vec3 rd, float time)
{
    ...
    float cl = 1.0 * (sin(uv.x) + sin(uv.y)); uv = mul(uv,mat2(0.8, 0.6, -0.6, 0.8)) * 2.1;
    ...
    if (res.y > -0.5)
    {
        ...
        else // terrain
        {
            ...
            col *= mix(1.0, vec3(0.5, 0.5, 0.4), mark);
            ks *= 1.0 - 0.5 * mark;
        }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // ray direction
    vec3 rd = mul(ca , normalize(vec3(p, 1.8)));
