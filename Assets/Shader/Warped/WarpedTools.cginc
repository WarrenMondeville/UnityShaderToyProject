...
float hash21(float2 p) { return frac(sin(dot(p, float2(27.609, 57.583))) * 43758.5453); }

// vec3 to float.
float hash31(float3 p) {
    return frac(sin(dot(p, float3(12.989, 78.233, 57.263))) * 43758.5453);
}
...
float2 path(in float z) {
    ...
    //return vec2(0);
    return float2(3. * sin(z * .1) + .5 * cos(z * .4), 0);
}
sampler2D iChannel0;

// 2D texture function.
//
float3 getTex(in float2 p) {

    // Strething things out so that the image fills up the window.
    //p *= vec2(iResolution.y/iResolution.x, 1);
    float3 tx = tex2D(iChannel0, p / 8.).xyz;
    //vec3 tx = textureLod(iChannel0, p, 0.).xyz;
    return tx * tx; // Rough sRGB to linear conversion.
}

// Height map value, which is just the pixel's greyscale value.
float hm(in float2 p) { return dot(getTex(p), float3(.299, .587, .114)); }
...
float opExtrusion(in float sdf, in float pz, in float h, in float sf) {
    ...
    float2 w = float2(sdf, abs(pz) - h - sf / 2.);
...
float2 skewXY(float2 p, float2 s) {

    return mul(float2x2(1, -s.y, -s.x, 1),p);
}

// Unskewing coordinates. "s" contains the X and Y skew factors.
float2 unskewXY(float2 p, float2 s) {

    return mul(inverse(float2x2(1, -s.y, -s.x, 1)), p);
}
...
float4 blocks(float3 q) {
    ...
    const float2[4] ps4 = float2[4](float2(-.5, .5), float2(.5), float2(.5, -.5), float2(-.5));
