...
vec4 Box4(vec4 p_x, vec4 p_y, vec4 p_z, vec4 b_x, vec4 b_y, vec4 b_z)
{
	...
	return min(max(d_x, max(d_y, d_z)), vec4(0, 0, 0, 0)) + len;
}
...
vec4 Box4Y(vec4 p_x, vec4 p_y, vec4 p_z, vec4 b_x, vec4 b_y, vec4 b_z, out vec4 Y)
{
	...
	return min(max(d_y, max_dxz), vec4(0, 0, 0, 0)) + len;
}
...
vec2 BrickSDFY(vec3 p)
{
	...
	vec4 py = vec4(p.y, p.y, p.y, p.y);// + vec4(0., 	0.,		0., 	0.);
...
float AddStuds(vec2 D, vec3 p)
{
	...
	//studs
	vec3 q = p;
	q.xz = Rep(q.xz - brick_w, vec2(2. * brick_w, 2. * brick_w));
...
#ifdef LOGO	
	d -= lego(10. * (fract(p.xz * vec2(0.5 / brick_w, 0.5 / brick_w) + 0.5) - 0.5)) * .05;
#endif
...
vec3 Light(vec3 X, vec3 n, vec3 V)
{
	...
	vec3 c = max(dot(lightDir, n), 0.);

	c = pow(texture(iChannel0, n).xyz, 2.2);
	...
	if (AddStuds(BrickSDFY(X), X) < mat_test)
		c *= pow(vec3(245. / 255., 205. / 255., 47. / 255.), 2.2);  //yellow
	else if (AddStuds(RedBrickSDFY(X), X) < mat_test)
		c *= pow(vec3(196. / 255., 40. / 255., 27. / 255.), 2.2); //red
	else if (mX < 30. * brick_w)
		c *= pow(vec3(161. / 255., 165. / 255., 162. / 255.), 2.2); //gray
	else if (mX < 44. * brick_w)
		c *= pow(vec3(13. / 255., 105. / 255., 171. / 255.), 2.2);  //blue
	else
		c *= pow(vec3(40. / 255., 127. / 255., 70. / 255.), 2.2);  //green
#endif	
