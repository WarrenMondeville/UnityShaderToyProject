...
// Mouth chomp!
#define sun vec2(-0.1, 0.6)
#define time (iTime+32.2)
#define lightDir normalize(vec3(.15, .4, .3))
#define swim ((time*.3+sin(time*.5+5.0)*.3)*3.0)
float height;
float pupilPos;
#define csb(f, con, sat, bri) mix(.5, mix(dot(vec3(.2125, .7154, .0721), f*bri), f*bri, sat), con)
...
vec3 GetColour(vec4 p, vec3 n, vec3 org, vec3 dir)
{
	...
	vec2 wat = p.xz * 5.3;
	wat += (texture(iChannel0, (wat * 5.0 + time * .04) * .1).z -
		texture(iChannel1, wat * .3 - time * .03).y) * .4;
	float	i = texture(iChannel0, wat * .025).x;

	i = min(pow(max(0.0, i - .2), 1.0) * 1.0, .6) * .3;
	colour += vec3(i * .5, i, i) * max(n.y, 0.0);

	float shad = Shadow(p.xyz, lightDir);
	colour = mix(0.1, colour, min(shad + .4, 1.0));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	if (uv.y >= 0.0)
	{
		...
		wat += (texture(iChannel2, (wat * .03 + time * .01) * .1).z -
			texture(iChannel3, wat * .02 - time * .01).y) * .4;
		i = texture(iChannel3, wat * .02).x;
		col += i * max(abs(uv.y), 0.0);
	}
	else
	{
		// Do floor stuff...
	...
	bright *= max(0.0, texture(iChannel2, (uv * .3 - swim * .04)).y);
	col += clamp(bright, 0.0, 1.0) * .6;
	...
	for (float i = 0.0; i < 50.0; i += 1.0)
	{
		...
		pos += texture(iChannel3, (uv * .3 - time * .1 + (i / 80.0))).z * .05;
	...
	col = mix(col, .0, abs(uv.x) * abs(uv.y));
