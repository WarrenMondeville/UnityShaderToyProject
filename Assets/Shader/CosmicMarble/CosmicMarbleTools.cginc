...
void renderColor2(vec3 ro, vec3 rd, inout vec3 color, vec3 currPos)
{
    ...
    color = mix(refract(normal, rd, 0.5) * 0.5 + 0.5, color, rim);
...
void renderColor(vec3 ro, vec3 rd, inout vec3 color, vec3 currPos)
{
    ...
    color = mix(color, normal * 0.5 + 0.5, rim_distorted + 0.1);
    color += rim;
    //color = normal;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // create view ray
    vec3 rd = normalize(mul(vec3(p.xy, 2.0), camMat)); // 2.0 is the lens length
