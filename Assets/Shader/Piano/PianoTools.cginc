...
float obj1(in vec3 p)
{
    ...
    return udRoundBox(q, vec3(0.091, 0.075, 0.6) - 0.005, 0.01);
}
...
{
    float d1 = udRoundBox(p - vec3(0.0, 0.0, 1.7), vec3(5.4, 0.6, 1.0), 0.05);
    float d2 = udRoundBox(p - vec3(0.0, -0.3, 0.1), vec3(5.4, 0.3, 0.6), 0.05);
    float d3 = udRoundBox(p - vec3(0.0, -1.0, 2.5), vec3(5.4, 3.0, 1.0), 0.05);
    ...
    float d5 = udRoundBox(vec3(abs(p.x), p.y, p.z) - vec3(5.05, 0.0, 0.3), vec3(0.35, 0.2, 0.8), 0.05);
...
float obj6(in vec3 p)
{
    ...
    q.yz = mul(q.yz, mat2(0.9, -0.43, 0.43, 0.9));
    return 0.5 * udRoundBox(q, vec3(1.0, 0.7, 0.0), 0.01);
}
...
float obj8(in vec3 p)
{
    ...
    q.xz = mul(q.xz, mat2(0.9, 0.44, -0.44, 0.9));

    float y = 0.5 + 0.5 * sin(8.0 * q.x) * sin(8.0 * q.z);
    y = 0.1 * pow(y, 3.0) * smoothstep(0.1, 0.4, q.y);
    float d = udRoundBox(q, vec3(1.5, 0.25, 0.6), 0.3);
    d += y;
...
float obj7(in vec3 p)
{
    ...
    return max(0.5 * udRoundBox(r, vec3(0.05, 0.0, 0.38), 0.08), sdBox(q, vec3(0.75, 1.0, 1.0)));
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (int k = 0; k < 2; k++)
    {
        ...
        if (tmat.y > -0.5)
        {
            ...
            vec3 lin = 0.0;
            lin = 4.00 * dif * vec3(1.5, 0.85, 0.55) * pow(sha, vec3(1.0, 1.2, 1.4));
