...
float dirt(vec2 uv, float n)
{
    ...
    vec2 rnd = hash(st);
    return Bokeh(p, vec2(0.5, 0.5) + vec2(0.2, 0.2) * rnd, 0.05, abs(rnd.y * 0.4) + 0.3, 0.25 + rnd.x * rnd.y * 0.2);
}
...
#define INTERVAL 3.0
    ...
    for (int i = 0; i < N; i++) {
        ...
        col = mix(col, (18.0 - t) / 18.0, dirt(mod(uv * max(0.0, t) * 0.1 + vec2(.2, -.2) * time, 1.2), 3.5));

        t = ii * INTERVAL - mod(time + INTERVAL * 0.5, INTERVAL);
        col = mix(col, 18.0 - t / 18.0 * vec3(0.7, 0.8, 1.0) * 1.3, tex(uv * max(0.0, t), 4.45));
        ...
        col = mix(col, mix((18.0 - t) / 18.0, (18.0 - t) / 18.0 * vec3(0.7, 0.5, 1.0) * 3.0, rr), tex(uv * 2.0 * max(0.0, t), 0.27 + (2.0 * rr)));
