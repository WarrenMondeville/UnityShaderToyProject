...
float map(vec3 p) {
    ...
    p.xy = mul(rot2(getTwist(p.z)),p.xy);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (t < FAR) {
        ...
        vec3 txP = sp;
        txP.xy -= path(txP.z);
        txP.xy = mul(rot2(getTwist(txP.z)), txP.xy);
#ifdef PTH_INDPNT_GRD
        txP.xy += path(txP.z);
#endif
