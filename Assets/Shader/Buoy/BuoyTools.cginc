...
vec2 Noise(in vec3 x)
{
	...
	vec4 rg = textureLod(iChannel0, (uv + f.xy + 0.5) / 256.0);
...
vec2 NoisePrecise(in vec3 x)
{
	...
	vec4 rg = mix(mix(
		textureLod(iChannel0, (uv + 0.5) / 256.0),
		textureLod(iChannel0, (uv + vec2(1, 0) + 0.5) / 256.0),
		f.x),
		mix(
			textureLod(iChannel0, (uv + vec2(0, 1) + 0.5) / 256.0),
			textureLod(iChannel0, (uv + 1.5) / 256.0),
			f.x),
		f.y);
...
vec4 Noise(in vec2 x)
{
	...
	vec2 uv = p.xy + f.xy;
	return textureLod(iChannel0, (uv + 0.5) / 256.0);
}

vec4 Noise(in ivec2 x)
{
	return textureLod(iChannel0, (vec2(x) + 0.5) / 256.0);
}

vec2 Noise(in ivec3 x)
{
	vec2 uv = vec2(x.xy) + vec2(37.0, 17.0) * float(x.z);
	return textureLod(iChannel0, (uv + 0.5) / 256.0).xz;
}
...
vec3 ShadeBoat(vec3 pos, vec3 ray)
{
	...
	albedo = mix(.04, albedo, smoothstep(.25 - aa, .25, abs(pos.y)));
	albedo = mix(mix(1, .04, smoothstep(-aa * 4.0, aa * 4.0, cos(atan(pos.x, pos.z) * 6.0))), albedo, smoothstep(.2 - aa * 1.5, .2, abs(pos.y)));
	albedo = mix(.04, albedo, smoothstep(.05 - aa * 1.0, .05, abs(abs(pos.y) - .6)));
...
vec3 ShadeOcean(vec3 pos, vec3 ray, in vec2 fragCoord)
{
	...
	// foam
	col = mix(col, 1, WaveCrests(pos, fragCoord));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec3 pos, ray;
	CamPolar(pos, ray, 0, camRot, 3.0, 1.0, fragCoord);
