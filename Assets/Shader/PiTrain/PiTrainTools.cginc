...
#define POW10_PER_COMPONENT 7
#define BASE_FOR_NUMBER 10000000
#define MAX_DIGIT 60
#define MAX_1OVER3_TERMS 122
#define MAX_2OVER11_TERMS 93
...
float GetPositionAlongLineSegmentNearestToPoint(vec2 line_segment_a, vec2 line_segment_b, vec2 tpoint)
{
	vec2 p = tpoint - line_segment_a;
...
float GetPositionAlongLineNearestToPoint(vec2 line_segment_a, vec2 line_segment_b, vec2 tpoint)
{
	vec2 p = tpoint - line_segment_a;
	vec2 l = line_segment_b - line_segment_a;
...
float GetDistSqFromLineSegmentToPoint(vec2 line_segment_a, vec2 line_segment_b, vec2 tpoint)
{
	float t = GetPositionAlongLineSegmentNearestToPoint(line_segment_a, line_segment_b, tpoint);

	vec2 to_nearest = tpoint - mix(line_segment_a, line_segment_b, t);
...
vec2 Box(vec2 box_origin, vec2 box_dx, vec2 box_dy, vec2 box_dims, float thickness, float roundness, vec2 uv, float pixel_uv_size)
{
	...
	if (r.x == r_in_box.x && r.y == r_in_box.y)
	{
		...
		if (sign(r - corner_sphere).x == to_corner.x && sign(r - corner_sphere).y == to_corner.y)
		{
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	if (time < 79.0)
	{
		...
		// Which digit of pi?
		float tline = -floor(found_base.y);
		float column = floor(found_base.x);

		if (tline >= 0.0 && column >= 0.0)
		{
			...
			if (column == 0.0)
			{
				...
				{
					nth_digit = tline * DIGITS_PER_LINE + (column - 1.0);
				}
	...
	if (uv.x > SCREEN_HEIGHT * -0.7 && uv.y > SCREEN_HEIGHT * 0.3 && uv.y < SCREEN_HEIGHT * 0.9)
	{
		for (int i = 0; i < NUM_DIGITS_IN_FLIGHT; i++)
		{
			...
			else
			{
				...
				int tline;
				int column;
				DivMod(int(puff_digit), int(DIGITS_PER_LINE), tline, column);
#else
				float tline;
				float column;
				DivMod(puff_digit, DIGITS_PER_LINE, tline, column);
#endif

				digit_dest_uv = DEST_DIGIT_UV + vec2((float(column) + 1.0) * KERNING - (1.0 - KERNING) * 0.5, -float(tline));
			}
