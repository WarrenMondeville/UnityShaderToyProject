...
#define minAmount -0.16
#define maxAmount 1.3
#define PI 3.14159
#define scale 512.0
...
float cylinderAngle;
#define cylinderRadius 1. / PI / 2.0

vec3 hitPoint(float hitAngle, float yc, vec3 tpoint, mat3 rrotation)
{
    float hitPoint = hitAngle / (2.0 * PI);
    tpoint.y = hitPoint;
    return mul(tpoint, rrotation);
}
...
float distanceToEdge(vec3 tpoint)
{
    float dx = abs(tpoint.x > 0.5 ? 1.0 - tpoint.x : tpoint.x);
    float dy = abs(tpoint.y > 0.5 ? 1.0 - tpoint.y : tpoint.y);
    if (tpoint.x < 0.0) dx = -tpoint.x;
    if (tpoint.x > 1.0) dx = tpoint.x - 1.0;
    if (tpoint.y < 0.0) dy = -tpoint.y;
    if (tpoint.y > 1.0) dy = tpoint.y - 1.0;
    if ((tpoint.x < 0.0 || tpoint.x > 1.0) && (tpoint.y < 0.0 || tpoint.y > 1.0)) return sqrt(dx * dx + dy * dy);
    return min(dx, dy);
}
...
vec4 seeThrough(float yc, vec2 p, mat3 rotation, mat3 rrotation)
{
    ...
    vec3 tpoint = hitPoint(hitAngle, yc, mul(vec3(p, 1.0), rotation), rrotation);
    if (yc <= 0.0 && (tpoint.x < 0.0 || tpoint.y < 0.0 || tpoint.x > 1.0 || tpoint.y > 1.0))
        return bgColor;
    if (yc > 0.0)
        return textureLod(iChannel1, p);
    vec4 color = textureLod(iChannel1, tpoint.xy);
    vec4 tcolor = 0.0;
    return antiAlias(color, tcolor, distanceToEdge(tpoint));
}

vec4 seeThroughWithShadow(float yc, vec2 p, vec3 tpoint, mat3 rotation, mat3 rrotation)
{
    float shadow = distanceToEdge(tpoint) * 30.0;
...
vec4 backside(float yc, vec3 tpoint)
{
    vec4 color = textureLod(iChannel1, tpoint.xy);
...
vec3 effect(vec2 uv)
{
    ...
    bgColor = textureLod(iChannel2, uv).rgga;
    ...
    vec3 tpoint = mul(vec3(uv, 1.0), rotation);
    float yc = tpoint.y - cylinderCenter;
    ...
    else
    {
        ...
        {
            tpoint = hitPoint(hitAngle, yc, tpoint, rrotation);
            if (tpoint.x < 0.0 || tpoint.y < 0.0 || tpoint.x > 1.0 || tpoint.y > 1.0)
            {
                color = seeThroughWithShadow(yc, uv, tpoint, rotation, rrotation);
            }
            else
            {
                color = backside(yc, tpoint);
                vec4 otherColor;
                if (yc < 0.0)
                {
                    float shado = 1.0 - (sqrt(pow(tpoint.x - 0.5, 2.0) + pow(tpoint.y - 0.5, 2.0)) / 0.71);
                ...
                {
                    otherColor = textureLod(iChannel1, uv);
                }
