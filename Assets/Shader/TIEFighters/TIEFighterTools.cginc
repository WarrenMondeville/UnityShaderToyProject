...
Hit sdWings(vec3 p) {
	...
	Hit tHit;
	tHit.d = min(sdTri(p, vec3(0, 0, 0), vec3(2, 3, 0), vec3(-2, 3, 0)), sdTri(p, vec3(0, 0, 0), vec3(3.3, 0, 0), vec3(2, 3, 0))) - .03;
	tHit.id = 2;
	tHit.uv = p;
...
Hit sdTie(vec3 p) {
	...
	Hit tHit0;
	tHit0.d = min(d, sdTri(p, vec3(0,0,0), vec3(0, .8, 0), vec3(0, 0, 2)) - .05);
	tHit0.id = 1;
	tHit0.uv = p;

	minH(h, tHit0); // Triangle cockpit supports.

	// Cockpit - Sphere.
	f = step(.75, p.y);

	Hit tHit1;
	tHit1.d = length(op) - .9 - .02 * (f + step(p.y, .03) + f * step(p.z, .1));
	tHit1.id = 6;
	tHit1.uv = p;
	minH(h, tHit1);
	...
	Hit tHit2;
	tHit2.d = max(length(p) - .7, sdPlane(p + vec3(.77, 0, 0), vec3(mul(vec2(-1, 0), rot(.5)), 0)));
	tHit2.id = 3;
	tHit2.uv = p;
	minH(h, tHit2);

	Hit tHit3;
	tHit3.d = max(length(p) - .71, .45 - length(p.yz));
	tHit3.id = 5;
	tHit3.uv = p;
...
vec3 lights(vec3 p, vec3 rd, float d, Hit h) {
	...
	if (h.id == 4) { // Snow
		...
		if (h.id == 1 || h.id == 6) { // Metal
			...
			c = .005 + .045 * pow(abs(sin((h.uv.x - h.uv.y) * 12.)), 20.);
			sped = .2;
		}
		else if (h.id == 7) { // Gunz.
			c = .02;
		...
		else if (h.id == 3) c = .05; // Cockpit glass.
		else if (h.id == 5) c = .1; // Glass surround.
...
vec3 march(vec3 ro, vec3 rd) {
	...
	if (d > 600.)
		...
		c = mix(lights(p, rd, d, h), 1, smoothstep(200., 540., d));
	c += gg * vec3(0, 1, 0);
	...
	if (h.id == 3 || h.id == 1) {
		...
		c = mix(c, d > 300. ? 1 : lights(p, rd, d, h), alpha);
	}
...
void mainImage(out vec4 fragColor, vec2 fc)
{
	...
	vec3 lookAt = mix(vec3(0, 0, 6) - tiePos(0, T - .2), vec3(2.5, 0, 0), t),
		ro = mix(lookAt - vec3(0, 0, 1), vec3(4. + cos(T), sin(T) * .2, -8. + cos(T * .2) * 6.), t);

	fragColor = vec4(
		vignette(pow(march(ro,
			getRayDir(ro, lookAt, (fc - .5 * iResolution.xy) / iResolution.y)),
			.45), fc), 0);
}
