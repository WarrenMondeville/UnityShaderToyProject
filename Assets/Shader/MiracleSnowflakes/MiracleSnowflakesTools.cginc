...
#define radius 0.25 // radius of Snowflakes. maximum for this demo 0.25.
float zoom = 4.0; // use this to change details. optimal 0.1 - 4.0.

vec3 light = vec3(0.0, 0.0, 1.0);
vec2 seed = vec2(0.0, 0.0);
float iteratorc = 15.0;
float powr;
float res;

#define NC0 vec4(0.0, 157.0, 113.0, 270.0)
#define NC1 vec4(1.0, 158.0, 114.0, 271.0)

float4 hash4(float4 n) { return fract(sin(n) * 1399763.5453123); }
float noise2(float2 x)
{
    ...
    float4 h = hash4(n + vec4(NC0.xy, NC1.xy));
    float2 s1 = mix(h.xy, h.zw, f.xx);
    return mix(s1.x, s1.y, f.y);
}

float noise222(float2 x, float2 y, float2 z)
{
    float4 lx = vec4(x * y.x, x * y.y);
    float4 p = floor(lx);
    float4 f = fract(lx);
    f = f * f * (3.0 - 2.0 * f);
    float2 n = p.xz + p.yw * 157.0;
    float4 h = mix(hash4(n.xxyy + NC0.xyxy), hash4(n.xxyy + NC1.xyxy), f.xxzz);
...
float noise3(float3 x)
{
    ...
    float4 s1 = mix(hash4(n + NC0), hash4(n + NC1), f.xxxx);
    return mix(mix(s1.x, s1.y, f.y), mix(s1.z, s1.w, f.y), f.z);
}
float2 noise3_2(float3 x) { return vec2(noise3(x), noise3(x + 100.0)); }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (int i = 0; i < layersblob; i++) {
        ...
        if (noise3(seedn) > 0.2 && i<layers) {
            ...
            mr = mul(mat3(vec3(coss.z, sins.z, 0.0), vec3(-sins.z, coss.z, 0.0), vec3(0.0, 0.0, 1.0)), mr);

            light = mul(normalize(vec3(1.0, 0.0, 1.0)), mr);
            ...
            vec4 cc = filterFlake(fragColor, mul((pos + addpos), mr), mul(ray, mr), mul(ray1, mr), mul(ray2, mr));
