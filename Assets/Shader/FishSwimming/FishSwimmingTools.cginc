...
#define NUMI 7
#define NUMF 7.0
	vec3 p1 = a; vec3 d1 = 0.0;
	vec3 p2 = a; vec3 d2 = 0.0;
	vec3 mp = a;
...
vec3 sdSeaBed(in vec3 p)
{
	float h = 1.0;
	vec3 q = p;
	float th = smoothstep(0.1, 0.4, textureLod(iChannel0, 0.002 * q.xz).x);
	float rr = smoothstep(0.2, 0.5, textureLod(iChannel1, 2.0 * 0.02 * q.xz).y);
	h = 0.9 + (1.0 - 0.6 * rr) * (1.5 - 1.0 * th) * 0.1 * (1.0 - textureLod(iChannel0, 0.1 * q.xz).x);
	h += th * 1.25;
	h -= 0.24 * rr;
	h *= 0.75;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	if (tmat.z > -0.5)
	{
		...
		if (tmat.z < 0.5)
		{
			...
			mate.xyz *= mix(1.0, vec3(0.2, 0.2, 0.2) * 1.5, rr);
		...
		else
		{
			...
			mate.xyz = mix(mate.xyz, mix(te.x * 0.5 + 1.5,
			...
			// stripes
			mate.xyz = mix(mate.xyz, (te.x + 0.5) * 1.0 * 0.5, 0.75 * smoothstep(0.5, 1.0, sin(1.0 * te.x + tmat.y * 100.0 + 13.0 * nor.y)) * smoothstep(0.0, 0.5, nor.y));
			...
			mate.xyz *= 0.5 + 0.5 * 1.0 * (1.0 - pa);

			// eye
			float r = length(vec2(5.0 * tmat.y, pos.y) - vec2(0.5, 0.13));
			r /= 1.2;
			mate.xyz = mix(mate.xyz, 1.5 * clamp(1.0 - r * 4.0, 0.0, 1.0), 0.5 * (1.0 - smoothstep(0.08, 0.09, r)));
			mate.xyz *= smoothstep(0.03, 0.05, r);
			mate.xyz += 4.0 * (1.0 - smoothstep(0.0, 0.1, r)) * pow(texture(iChannel1, 4.0 * vec2(0.2 * fishPos.z + 4.0 * tmat.y, pos.y)).x, 2.0);
			r = length(vec2(5.0 * tmat.y, pos.y) - vec2(0.48, 0.14));
			mate.xyz = mix(mate.xyz, 2.0, (1.0 - smoothstep(0.0, 0.02, r)));
		...
		lin += 2.0 * spe * 1.0 * sha * (0.3 + 0.7 * fre);
