#include "CyberTruckCommon.cginc"
...
vec2 GetDist(vec3 p) {
    ...
    if (p.z > 0.) wp.xz = mul(wp.xz, Rot(.3 * sign(p.x)));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    ro.xz = mul(ro.xz, Rot(-m.x * 6.2831 + t * .3 + 1.));
    ro.y = max(ro.y, .1);
    ...
    // neon tubes
    float climax = 1. + step(35., _Time.y) * 5. * step(_Time.y, 52.);
    ...
    beamStart.xz = mul(beamStart.xz, rot);
    beamEnd.xz = mul(beamEnd.xz, rot);
