...
#define maxdist 50.
float objcol, flo;
#define ldir normalize(vec3(4., -1.5, -1.))
#define lcol vec3(.7, .3, .2)
#define ilcol vec3(.6, .5, 1.)
...
vec3 rotate(vec3 p) {
    p.xz = mul(p.xz, rot(iTime));
    p.yz = mul(p.yz, rot(iTime * .5));
    return p;
}
...
vec3 shade(vec3 p, vec3 n, vec3 dir) {
    ...
    vec3 col = ocol * vec3(1.5, 1.5, 1.5) - f;
...
vec3 march(vec3 from, vec3 dir) {
    ...
    vec3 p = from, refrom = from, pl = p, col = 0., savecol = col, odir = dir;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    from.xz = mul(from.xz, rot(iTime * .2));
    dir.xz = mul(dir.xz, rot(iTime * .2));
    dir.x += sin(iTime * .5) * .3;
    dir = normalize(dir);
