...
vec3 hash31(float p) {
    ...
    vec3 p3 = fract(p * vec3(.1031, .11369, .13787));
...
void CameraSetup(vec2 uv, vec3 position, vec3 lookAt, float zoom) {
    ...
    cam.tup = cross(cam.forward, cam.left);
    cam.zoom = zoom;

    cam.center = cam.p + cam.forward * cam.zoom;
    cam.i = cam.center + cam.left * uv.x + cam.tup * uv.y;
...
de map(in vec3 p) { return map(p, vec3(1., 1., 1.)); }
...
vec3 FishMaterial(de o, vec3 nor, float fresnel, float spec, float occ, vec3 amb, vec3 ref, float u, vec3 pos) {
    ...
    vec3 bottomCol = vec3(1.3, 1.3, 1.3);
    ...
    vec3 headCol = mix(vec3(1.2, 1.2, 1.2), topCol, S(0., .5, o.d4) * S(1.1, .5, o.d4));
    headCol += 0.1 * (sin(o.a2 * 20. + o.d4) * sin(o.a2 * 3. + o.d4 * -4.) * SIN(o.d4 * 10.));

    headCol += (1. - fresnel) * ref.b * .2;
    vec3 mouthCol = vec3(1.3, 1.3, 1.3);
    ...
    vec3 ambient = mix(amb, vec3(occ, occ, occ), .5);
    col *= ambient;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec3 pos = mul(vec3(0., INVERTMOUSE * 10. * cos((m.y) * pi), -10.), rotX);
