...
#define miss 1E4
#define refrIndex 0.85
#define lightPos 2.0 * vec3(1.5, 2.0, 1.0)
#define skyCol1 pow(vec3(0.2, 0.4, 0.6), 0.25) * 1.0
#define skyCol2 pow(vec3(0.4, 0.7, 1.0), 2.0) * 1.0
#define sunCol vec3(8.0, 7.0, 6.0) / 8.0
...
vec3 skyColor(vec3 ro, vec3 rd) {
    ...
    if (tp > 0.0) {
        ...
        vec3  pcol = mix(0.4, 0.3, exp(-60.0 * pd));
...
vec3 effect(vec2 p, vec2 q) {
    ...
    ro.xz = mul(ro.xz, ROT(PI / 2.0 + sin(TIME * 0.05)));
    ro.yz = mul(ro.yz, ROT(0.5 + 0.25 * sin(TIME * 0.05 * sqrt(0.5)) * 0.5));
