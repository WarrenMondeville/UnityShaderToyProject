...
{
    vec3 col = 0.0;
    col += 0.5 * pow(fancyCube(iChannel1, d, 0.05, 5.0).zyx, 2.0);
    col += 0.2 * pow(fancyCube(iChannel1, d, 0.10, 3.0).zyx, 1.5);
    col += 0.8 * vec3(0.80, 0.5, 0.6) * pow(fancyCube(iChannel1, d, 0.1, 0.0).xxx, 6.0);
...
#define sph1 vec4(0.0, 0.0, 0.0, 1.0)
...
vec3 render(in vec3 ro, in vec3 rd)
{
    ...
    if (t > 0.0)
    {
        ...
        vec3 tnor = nor;
        tnor.xz = mul(tnor.xz, mat2(pr.x, -pr.y, pr.y, pr.x));

        float am2 = 0.08 * iTime - 1.0 * (1.0 - nor.y * nor.y);
        pr = vec2(cos(am2), sin(am2));
        vec3 tnor2 = nor;
        tnor2.xz = mul(tnor2.xz, mat2(pr.x, -pr.y, pr.y, pr.x));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec3 rd = normalize(mul(vec3(p, -2.0), cam));
...
void mainVR(out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir)
{
    ...
    fragColor = vec4(render(ro + mul(fragRayOri, cam),
        mul(fragRayDir,cam)), 1.0);
