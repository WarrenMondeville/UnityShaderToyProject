...
vec3 face(vec2 p)
{
    ...
    // Eyes
    c = mix(c, 0, linstep(.02, .01, box(p - vec2(.45, .3), vec2(.15, .17))));
    c = mix(c, 1, linstep(.02, .01, box(p - vec2(.4, .39), vec2(.04, .04))));

    // Nose
    c = mix(c, 0, linstep(.02, .01, box(p - vec2(0, -.02), vec2(.07, .03))));
...
vec3 sampleScene(vec2 coord)
{
    ...
    rd.xz = mul(mat2(cos(an), sin(an), sin(an), -cos(an)), rd.xz);
    ro.xz = mul(mat2(cos(an), sin(an), sin(an), -cos(an)), ro.xz);
    ...
    if (groundt < dt)
    {
        ...
        c *= mix(.7, 1., linstep(0., .1, box(rp.xz, vec2(1.,1.)) - .5));
    }
    else if (dt < 20.)
    {
        // SDF is nearer.
        ...
        if (bodyWithMouth(rp) > body(rp))
            ...
            // Tips of ears.
            c = mix(c, 0, linstep(0., .01, rp.y - 1.7));
        ...
        c *= mix(vec3(1, 1, .8) * .9, 1, smoothstep(.3, .31, l));
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Gamma correction.
    fragColor.rgb = pow(fragColor.rgb, 1. / 2.2);
}
