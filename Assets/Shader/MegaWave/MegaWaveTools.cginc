...
void mainImage(out vec4 outColor, in vec2 g)
{
	outColor = 0;
	//return;
	...
	vec3 sky = GetSky(rayDir, ligthDir, vec3(h * 5., h * 5., h * 5.));
	...
	if (d < dMax)
	{
		...
		outColor.rgb = 0.4 + 0.1 * r * vec3(1.0, 1.0, 1.0);
		...
		outColor.rgb *= brdf + dstepf;
		outColor.rgb = mix(outColor.rgb, sky, 1.0 - exp(-0.001 * d * d));
	}
	else
	{
		outColor.rgb = sky;
