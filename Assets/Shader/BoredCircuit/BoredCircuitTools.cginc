...
vec2 z, v;
#define e vec2(.00035, -.00035)
float t, b, tt, g;vec3 op, po, no, al, ld, cutP;
mat2 r2(float r) { return mat2(cos(r), sin(r), -sin(r), cos(r)); }
float bo(vec3 p, vec3 r) { p = abs(p) - r;return max(max(p.x, p.y), p.z); }
vec2 fb(vec3 p)
{
    cutP = op = p;
    cutP.xy = mul(r2(.785), cutP.xy);
...
vec2 mp(vec3 p)
{
    ...
    for (int i = 0;i < 4;i++) {
        ...
        np.xy = mul(r2(.785), np.xy);
        bp.xy = mul(r2(.785), bp.xy);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    tt = mod(iTime, 56.52);
    b = ceil(sin(tt * .5));
    vec3 ro = mix(vec3(1,1,1), vec3(-2, 3, 1), b) * vec3(sin(tt * .5 - .75) * 6., 7.5, 30. + sin(tt * .15) * 70.),
        cw = normalize(vec3(0, 0, 20. + sin(tt * .5) * 20.) - ro), cu = normalize(cross(cw, vec3(0, 1, 0))), cv = normalize(cross(cu, cw)),
        rd = mul(mat3(cu, cv, cw) , normalize(vec3(uv, .75))), co, fo;
    ...
    if (z.y > 0.) {
        ...
        al = mix(vec3(0,0,0), vec3(.05, .15, .35), lines);
        float spo = exp2(7. - lines * 5.);
        if (z.y < 5.)al = vec3(0.,0.,0.);
    ...
    fragColor = vec4(pow(co + g * .4 * vec3(.1, .2, .5), vec3(.45,.45,.45)), 1);
}