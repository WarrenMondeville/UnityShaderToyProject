...
vec4 invsf(vec3 p, float n)
{
    ...
    vec2 c = floor(mul(iB, vec2(phi, p.z - m)));
    float d = 0.;
    vec4 res = 0;
    for (int s = 0; s < 4; s++)
    {
...
vec2 Virus(vec3 p, float atime) {
    ...
    p.xy = mul(p.xy, Rot(0.04 * atime));
    p.xz = mul(p.xz, Rot(0.06 * atime));
...
vec3 calcNormal(in vec3 pos, float time, float quality) {
    ...
    {
        vec3 e1 = 0.5773 * (2.0 * vec3((((i + 3) >> 1) & 1), ((i >> 1) & 1), (i & 1)) - 1.0);
        n += e1 * map(pos + quality * e1, time).x;
    }
    return normalize(n);
}
...
vec4 render(in vec3 ro, in vec3 rd, float time)
{
    ...
    if (res.y > 0.5) {
        ...
        float fft = texelFetch(iChannel0, ivec2(3., 0.75)).x;
        fft = smoothstep(0.8, 1., fft) * 2.;

        float fft1 = texelFetch(iChannel0, ivec2(5., 0.75)).x;
        fft1 = smoothstep(0.9, 1., fft1) * 1.5;

        float fft2 = texelFetch(iChannel0, ivec2(6., 0.75)).x;
        fft2 = smoothstep(0.9, 1., fft2);

        float fft3 = texelFetch(iChannel0, ivec2(7., 0.75)).x;
        ...
        vec3 specular = max(.30, dot(light_pos, ref));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    ro.xy = mul(ro.xy, Rot(sin(t * .1) * PI));
    ro.xz = mul(ro.xz, Rot(sin(t * .1) * PI));
    ro.yz = mul(ro.yz, Rot(sin(t * .1) * PI));
