...
vec3 shading(vec4 hit, vec3 ro, vec3 rd)
{
    ...
    if (hit.y < 1.5)
    {
        ...
        surfCol = mix(vec3(0.1, 0.58, 0.85), 0.0, h);
        surfCol *= detail;
...
vec3 render(in vec2 fragCoord)
{
    ...
    col += glowCol;
    // fog
    col = mix(col, 0.0, 1.0 - exp(-0.001 * hit.x * hit.x));
    return col;
