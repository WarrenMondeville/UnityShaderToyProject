...
vec2 map(vec3 p)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		vec2  r = mul(p.xz, ma);//-vec2(1.0,0.0));
		...
		co = cos(0.2 + an);
		si = sin(0.2 + an);
		q.xy = mul(q.xy, mat2(co, -si, si, co));
...
#define lig normalize(vec3(0.5, 0.35, -0.7))
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	if (tmat.z > -0.5)
	{
		...
		else if (tmat.z < 3.5)
		{
			...
			mate.xyz += e * 0.2 * vec3(0.8, 1.0, 0.5);
		}
		...
		// lights
		vec3 brdf = 0.0;
		brdf += 8.0 * dif * vec3(1.20, 0.90, 0.50) * pow(sha, vec3(1.0, 1.2, 1.5)) * (0.8 + 0.2 * occ);
		...
		col = mate.xyz * brdf + 2.5 * mate.w * spe * sha;
