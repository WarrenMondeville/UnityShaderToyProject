//https://www.shadertoy.com/view/3ddfDS
...
vec3 getMouse(vec3 ro) {
    float x = iMouse.xy == 0 ? .0 :
        (M.y / R.y * .25 - .125) * PI;
    float y = iMouse.xy == 0 ? .0 :
        -(M.x / R.x * .25 - .125) * PI;
...
vec3 render(in vec3 ro, in vec3 rd, in vec2 uv) {
    ...
    if (ray.x < MAX_DIST) {
        ...
        vec3 diff = .9 * getDiff(p, lpos2, ray.x);
        diff += .9 * getDiff(p, lpos1, ray.x);
        float ao = calAO(p, n);
