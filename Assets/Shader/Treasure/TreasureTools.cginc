...
vec4 tetcol(vec3 p, vec3 offset, float scale, vec3 col) {
    ...
    return vec4(col, (cube(z.xyz, 1.5)) / z.w);
}

float bpm = 128.;
vec4 dist(vec3 p, float t) {
    p.xy = mul(p.xy, rot(PI));
    p.xz = pmod(p.xz, 24.);
    p.x -= 5.1;
    p.xy = mul(p.xy, rot(0.3));
    p.xz = mul(p.xz, rot(0.25 * PI));
    p.yz = mul(p.yz, rot(PI * 0.5));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (col.r < 0.1 && (col.b < 0.1 && col.g < 0.1)) col = 0.;
