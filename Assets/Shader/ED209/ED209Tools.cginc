...
MarchData headLower(vec3 p) {
    ...
    // Add the side panels.
    float roof = max(max(headVisor((p + vec3(0.0, 0.01, 0.0)) * 0.95, 1.0, 0.0).d, p.y - 0.35), p.y * 0.625 - p.z - 0.66);
    r.d = min(r.d, roof);

    // 'Wings'.
    p.xy = mul(rot(0.075 * (gunsUp - 1.0) * sign(p.x)),p.xy);
    p.x = abs(p.x) - 1.33;
    p.y -= 0.1 - p.x * 0.1;
...
MarchData gunPod(vec3 p) {
    ...
    vec3 pp = p;
    pp.z = abs(pp.z) - 0.5;
    r.d = sdCappedCone(pp, 0.0, vec3(0.0, 0.0, -chamfer), rr - chamfer, rr);
...
MarchData arms(vec3 p) {
    ...
    p.x = abs(p.x);
    p.yz += vec2(0.24, -0.0);
    p.xy = mul(rot(0.15 * (gunsUp - 1.0)),p.xy);

    // Shoulder and forearm.
    const vec3 elbow = vec3(1.5, 0.0, 0.0), wrist = elbow - vec3(0.0, 0.0, 0.3);
    r.d = min(sdCapsule(p, 0.0, elbow, 0.2), sdCapsule(p, elbow, wrist, 0.2));
...
float foot(vec3 p) {
    p.z += 0.8;
    p.yz = mul(rot(0.86),p.yz);

    float d = toe(p);
    p.xz = mul(rot(1.57),p.xz);
    p.x -= 0.43;
    p.z = 0.25 - abs(p.z);
...
MarchData waist(vec3 p) {
    ...
    p.y += 0.65;
    p.yz = mul(rot(-0.2),p.yz);
    float legAngle = legWalkAngle(1.0);
    p.xy = mul(rot(legAngle * 0.3),p.xy);
    ...
    // Hips.
    pp.x = abs(pp.x);
    pp.yz = mul(rot(0.2 - 3.141 / 4.0 + legAngle * sign(p.x)),pp.yz);
    pp.x -= 0.98;
...
MarchData legs(vec3 p) {
    ...
    float legAngle = legWalkAngle(1.0);
    p.z += 0.27;
    p.yz = mul(rot(legAngle * sign(p.x)),p.yz);
    p.z -= 0.27;
    ...
    pp.x = abs(pp.x);
    pp.y += 0.48;
    pp.yz = mul(rot(0.2 - 3.141 / 4.0),pp.yz);
    pp.x -= 0.98;
...
MarchData ed209(vec3 p) {
    ...
    p.yz = mul(rot(0.1 * (-edDown + legWalkAngle(2.0) + smoothstep(0.0, 1.0, clamp((stretch - 0.5) * 6.0, 0.0, 1.0)) - 1.0)),p.yz); // 0.5-0.66
    p.xz = mul(rot(edTwist * 0.2),p.xz);
...
MarchData room(vec3 p) {
    ...
    p.x -= frameInner.x;
    p.xz = mul(rot(doorOpen * 2.1),p.xz);
    p.x += doorWidth;
...
vec3 applyLighting(vec3 p, vec3 rd, float d, MarchData data) {
    ...
    return mix(data.mat * lig, 0.01, fre) * fog;
}
...
vec3 getSceneColor(vec3 ro, vec3 rd) {
    ...
    for (float steps = 0.0; steps < MAX_STEPS; steps++) {
        ...
        if (d > MAX_DIST)
            return 0.0; // Distance limit reached - Stop.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Output to screen.
    fragColor = vec4(vignette(pow(col * dim, 0.4545), fragCoord), 1.0);
}
