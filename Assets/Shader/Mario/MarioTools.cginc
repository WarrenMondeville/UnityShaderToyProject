...
#define iTime fmod(_Time.y,50.0)

#define MARIO_SPEED 89.0
#define GOOMBA_SPEED 32.0
#define INTRO_LENGTH 2.0
...
void DrawGame( inout float3 color, float time, float pixelX, float pixelY, float screenWidth, float screenHeight )
{
    ...
    float marioFrame 	= floor( fmod( _MoveX/10, 3.0 ) );
    if (_BigKey > 0)
    {
        marioFrame += 4;
    }
    if ( marioX >= 0.0 && marioX <= 15.0 && cameraX < 3152.0 )
    {
        if (_MoveSpeed > 0)
        {
            ...
            SpriteMario(color, 15 - marioX, marioY, marioFrame);
        }
    }
}
