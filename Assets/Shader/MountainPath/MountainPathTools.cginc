...
float map(vec3 p) {
    ...
    float ter = p.y - 3. + dot(sin(p * 3.14159 / 18. - cos(p.yzx * 3.14159 / 18.)), 3); // 6. smoothing factor.
...
float n2D(vec2 p) {
    ...
    return dot(mul(mat2(fract(sin(vec4(0, 41, 289, 330) + dot(p, vec2(41, 289))) * 43758.5453)), vec2(1. - f.y, f.y)), vec2(1. - f.x, f.x));
