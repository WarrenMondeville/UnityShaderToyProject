...
int mod3(int n)
{
    n %= 3; return (n < 0) ? n + 3 : n;

    // Other methods of computing mod3:
    //   1.  return (n<0) ? 2-((2-n)%3) : n%3;
    //   2.  n %= 3; n+=((n>>31)&3); return n;
    //   3.  return ((n%3)+3)%3;
    //   4.  return int[](1,2,0,1,2)[n%3+2];
}
...
vec3 render(in vec3 ro, in vec3 rd, in float time)
{
    ...
    if (t > 0.0)
    {
        ...
        // diffuse
        col = mate * pow(occ, vec3(0.95, 1.05, 1.1));
        ...
        // fog
        col = mix(col, 1.0, 1.0 - exp2(-0.00005 * t * t));
    }
...
#if AA>1
            float time = iTime - 0.5 * (1.0 / 24.0) * (float(m * AA + n) + frand()) / float(AA * AA);
#else
            float time = iTime;
#endif
...
#if AA>1
            vec3 fp = ro + rd * 17.0;
            ro.xy += 0.5 * (vec2(frand(), frand()) - 0.5);
            rd = normalize(fp - ro);
#endif
