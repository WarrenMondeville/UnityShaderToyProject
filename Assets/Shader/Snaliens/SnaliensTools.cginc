...
vec4 eye_shape(vec2 uv, float blur, float id)
{
    ...
    color -= mix(color, vec3(0.,0.,0.), 1. - innerMask);
    color = mix(color, vec3(1.,1.,1.), eye_whiteMask);

    float shade = min(1., (-eye / 0.12 / 0.2) * 0.5 + 0.6);
    shade *= (uv.y) / 0.25 + 0.9;
    color *= vec3(1,1,1) * shade;
...
float tree(vec2 uv)
{
    ...
    uv.y -= 0.35;
    uv = mul(uv, rot(0.01 + cos(iTime) * 0.1));
...
vec4 flr(vec2 uv)
{
    ...
    vec2 lv = mul((uv + vec2(-0.55, 0.)), rot(-0.12 + cos(iTime * 2.) * 0.01));
    lv.y += 0.2;
    lv *= 1.7;
    l += tree(lv);

    lv = mul((uv + vec2(0.55, 0.)),rot(0.12 + cos(iTime * 2. + 23.) * 0.1));
    lv *= 1.8;
    l += tree(lv);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec2 uv2 = mul((uv + vec2(0.2, 0.)), rot(0.4));
    vec4 b2 = body(uv2, c2, 0.2, blur, 2.);
    uv2 = mul((uv + vec2(-0.2, 0.)), rot(-0.4));
