#include "Link'sAwakeningCommon.cginc"
// The MIT License
...
vec2 map(in vec3 pos)
{
    ...
    //----- HEAD -----
    vec3 lHeadStart =mul((pos - vec3(0.0, 0.53, -0.00)), mul(rotation(X_AXIS, -0.12), rotation(Y_AXIS, 0.2)));
    ...
    if (res.x > lHeadBox)
    {
        ...
        //Main Head
        float   lHead = sdEllipsoid(mul((lHeadStart), rotation(X_AXIS, 0.4)), vec3(0.17, 0.14, 0.19));
        ...
        //Ears
        vec3	lEarsPos = mul((opMirrorX(lHeadStart) - vec3(0.2, 0.018, 0.02)), mul(rotation(Y_AXIS, 0.4), rotation(Z_AXIS, -0.4)));
        float 	lEars = sdEllipsoid(lEarsPos, vec3(0.13, 0.044, 0.03));
        //inner
        lEars = fOpIntersectionRound(lEars, -sdEllipsoid(mul(lEarsPos, rotation(Y_AXIS, -0.24)) - vec3(0.0, 0.0, -0.013), vec3(0.07, 0.018, 0.018)), 0.015);
        //punta
        ...
        //Eyes
        float lEyes = sdEllipsoid(mul((opMirrorX(lHeadStart) - vec3(0.055, 0.08, -0.165)), rotation(Z_AXIS, 0.05)), vec3(0.0195, 0.05, 0.02));
        lEyes = max(lEyes, lHead);
        res = opU(res, vec2(lEyes, 4.0));

        mat3 lRotZEB = rotation(Z_AXIS, -0.2);

        float lEyeBrows = sdEllipsoid(mul((opMirrorX(lHeadStart) - vec3(0.06, 0.115, -0.14)), lRotZEB), vec3(0.036, 0.012, 0.03));
        lEyeBrows = fOpIntersectionRound(lEyeBrows, -sdEllipsoid(mul((opMirrorX(lHeadStart) - vec3(0.058, 0.103, -0.14)), lRotZEB), vec3(0.036, 0.011, 0.03)), 0.003);
        ...
        //Hair
        {
            ...
            //Right bulto
            lHair = min(lHair, sdEllipsoid(mul((lHeadStart - vec3(0.06, 0.2, -0.05)), mul(rotation(Z_AXIS, 0.3), rotation(Y_AXIS, -0.4))), vec3(0.16, 0.07, 0.13)));

            //Right back bulto
            lHair = min(lHair, sdEllipsoid(mul((lHeadStart - vec3(0.12, 0.15, -0.00)), mul(rotation(Z_AXIS, 0.5), rotation(Y_AXIS, -0.4))), vec3(0.1, 0.07, 0.1)));

            //Left bulto
            lHair = min(lHair, sdEllipsoid(mul((lHeadStart - vec3(-0.12, 0.16, -0.03)), mul(rotation(Z_AXIS, -1.0), rotation(Y_AXIS, -0.4))), vec3(0.1, 0.07, 0.1)));

            //Patillas Left
            lHair = min(lHair, sdEllipsoid(mul((lHeadStart - vec3(-0.19, 0.0, -0.03)), rotation(Z_AXIS, (lHeadStart.y - 0.18) * -2.5)), vec3(0.03, 0.1, 0.03)));

            //Patillas Right
            lHair = min(lHair, sdEllipsoid(mul((lHeadStart - vec3(0.19, 0.0, -0.03)), rotation(Z_AXIS, (lHeadStart.y - 0.18) * 1.1)), vec3(0.03, 0.12, 0.03)));
        ...
        float lCap = sdEllipsoid(mul((lHeadStart - vec3(0.0, 0.09, -0.16)), lCapRot), vec3(0.23, 0.2, 0.65));
        lCap = fOpIntersectionRound(lCap, -lCapCutter, 0.02);

        res = opU(res, vec2(lCap, 3.0));

        float lBandCutter = sdEllipsoid(mul((lHeadStart - vec3(0.0, 0.11, 0.38)), rotation(X_AXIS, 0.5)), vec3(0.5, 0.8, 0.25));
    ...
    if (res.x > lBottomBox)
    {
        ...
        {
            vec3 lRightFootPos = mul((pos - vec3(0.09, 0.0, -0.03)), rotation(Y_AXIS, 0.2));
            float 	lShoes = sdCone(mul(lRightFootPos, rotation(Z_AXIS, -0.2)), 0.0, vec3(0.0, 0.06, 0.0), 0.04, 0.042);
            ...
            //Legs
            float lLegs = sdCone(mul(lRightFootPos, rotation(Z_AXIS, -0.2)) - vec3(0.0, 0.04, 0.0), 0.0, vec3(0.0, 0.06, 0.0), 0.038, 0.035);
            res = opU(res, vec2(lLegs, 8.0));
        }
        {

            vec3 lLeftFootPos = mul((pos - vec3(-0.06, 0.0, -0.03)), rotation(Y_AXIS, -1.2));
            float 	lShoes = sdCone(lLeftFootPos, 0.0, vec3(0.0, 0.06, 0.0), 0.04, 0.042);
            lShoes = opSmoothUnion(lShoes, sdEllipsoid(lLeftFootPos - vec3(0.0, 0.008, -0.07), vec3(0.045, 0.04, 0.048)), 0.05);
            //lShoes = opSmoothUnion(lShoes, sdEllipsoid( pos - vec3(-0.06, -0.001,  0.02), vec3(0.02 , 0.018, 0.015)), 0.08);
            res = opU(res, vec2(lShoes, 5.0));

            //Legs
            float lLegs = sdCone(lLeftFootPos - vec3(0.0, 0.04, 0.0), 0.0, vec3(0.0, 0.06, 0.0), 0.038, 0.035);
            res = opU(res, vec2(lLegs, 8.0));
        }
    }
    //----- END BOTTOM BODY ------
    ...
    if (res.x > lMiddleBox)
    {
        ...
        vec3 	lMangasPos = mul((opMirrorX(lTorsoPos) - vec3(0.09, 0.35, 0.0)), mul(rotation(Z_AXIS, -0.6), rotation(X_AXIS, pos.x * -2.2)));
        ...
        //Real shirt cuff
        lMangas = opSmoothUnion(lMangas, sdCone((lMangasPos - vec3(0.0, -0.106, 0.0)), 0.0, vec3(0.0, 0.045, 0.0), 0.032, 0.019), 0.03);
        ...
        //Arms
        float 	lArms = sdCone(lMangasPos - vec3(0.0, -0.12, 0.0), 0.0, vec3(0.0, 0.03, 0.0), 0.022, 0.02);
    ...
    //Sword
    vec3 lSwordPos = mul((pos - vec3(-0.146, 0.175, 0.02)), mul(rotation(Y_AXIS, M_PI * -0.2), mul(rotation(X_AXIS, M_PI * 0.5), rotation(Y_AXIS, M_PI * -0.28))));
    ...
    if (res.x > lSwordBox)
    {
        ...
        vec3 	lGuardSidesPos = mul((opMirrorX(lGuardPos) - vec3(0.05, 0.002, 0.0)), rotation(Z_AXIS, -0.5));
    ...
    //SHIELD
    vec3 	lShieldPos = mul((pos - vec3(0.2, 0.2, -0.15)), mul(rotation(Y_AXIS, M_PI * 0.18), rotation(X_AXIS, M_PI * -0.05))) * 0.74;
    ...
    if (res.x > lShieldBox)
    {
        ...
        lMetalDecorations = min(lMetalDecorations, sdCone((mul(lShieldPos, rotation(Z_AXIS, -1.0)) - vec3(0.02, -0.076, 0.0)), lConeSize.x, lConeSize.y, lConeSize.z));
        lMetalDecorations = min(lMetalDecorations, sdCone(mul(lShieldPos, rotation(Z_AXIS, -2.0)) - vec3(0.028, -0.125, 0.0), lConeSize.x, lConeSize.y, lConeSize.z));
        lMetalDecorations = min(lMetalDecorations, sdCone(mul((lShieldPos - vec3(0.0, 0.125, 0.0)), rotation(X_AXIS, -0.2)), 0.02, 0.02, 0.04));
        ...
        float	lBird = sdBox(mul((lShieldPos - vec3(-0.005, 0.026, -0.018)), rotation(Z_AXIS, -0.5)), vec3(0.013, 0.016, 0.04));
        lBird = min(lBird, sdBox(mul((lShieldPos - vec3(-0.008, -0.026, -0.018)), rotation(Z_AXIS, 0.3)), vec3(0.013, 0.025, 0.04)));
        ...
        lBird = min(lBird, sdBox(mul((lShieldPos - vec3(0.046, 0.025, -0.018)), rotation(Z_AXIS, 0.1)), vec3(0.018, 0.0035 + (lShieldPos.x * 0.1), 0.04)));
        lBird = min(lBird, sdBox(mul((lShieldPos - vec3(0.04, 0.008, -0.018)), rotation(Z_AXIS, 0.3)), vec3(0.015, 0.003 + (lShieldPos.x * 0.08), 0.04)));

        lBird = min(lBird, sdBox(mul((lShieldPos - vec3(0.032, -0.003, -0.018)), rotation(Z_AXIS, 0.7)), vec3(0.013, 0.002 + (lShieldPos.x * 0.06), 0.04)));

        lBird = max(lBird, lInnerShield - 0.0025);

        float   lRays = sdCone(mul((lShieldPos - vec3(0.04, 0.08, -0.018)), rotation(Z_AXIS, -0.2 - pow(lShieldPos.x * 5.5, 0.8))), 0.033, 0.01, 0.001);

        lRays = max(lRays, lInnerShield - 0.0025);

        //HANDLE        
        float lHandle = sdTorus(mul((lShieldPos - vec3(0.0, 0.0, 0.02)), rotation(Z_AXIS, M_PI * 0.5)), vec2(0.03, 0.008));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // ray direction
    vec3 rd = mul(normalize(vec3(p, 2.0)), ca);
    ...
    vec3 rdx = mul(normalize(vec3(px, 2.0)), ca);
    vec3 rdy = mul(normalize(vec3(py, 2.0)), ca);
